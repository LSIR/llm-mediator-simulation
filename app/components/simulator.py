...
def debate_simulator_page():
    ...
    with checks[1]:
        st.checkbox(
            "Unmediated",
            value=st.session_state.unmediated,
    ...
    with checks[2]:
        st.checkbox(
            "Mediated",
            value=st.session_state.mediated,
            on_change=flip_debate_type,
    ...
    with checks[3]:
        st.markdown(
            "Select the debate types you want to simulate, selecting both is valid."
        )
    ...
    for i, k in enumerate(st.session_state.metrics):
        with metrics_columns[i + 1]:
            st.checkbox(
                k.value[0],
                value=st.session_state.metrics[k],
    ...
    for col, debater in zip(cols, st.session_state.debaters):
        with col:
            with st.expander(
                f"{likert7_to_emoji(debater.topic_opinion.agreement)} **{debater.name}** ({debater.topic_opinion.agreement.value.capitalize()})"
            ):
                if debater.personality and debater.personality.demographic_profile:
                    ...
                    for (
                        characteristic,
                        value,
                    ) in debater.personality.demographic_profile.items():
                        st.markdown(
                            f"<li style='margin-bottom: 2px;'><b>{characteristic.value.capitalize()}:</b> {value}</li>",
    ...
    if (st.session_state.unmediated or st.session_state.mediated) and (
        "unmediated_debate" not in st.session_state
        or st.session_state.debate_topic
        != st.session_state.unmediated_debate.config.statement
        or st.session_state.num_debaters
        != len(st.session_state.unmediated_debate.debaters)
        or st.session_state.debater_model
        != st.session_state.unmediated_debate.debater_model.model_name
        or any(
            debater != st.session_state.unmediated_debate.debaters[i].config
            for i, debater in enumerate(st.session_state.debaters)
        )
        or (st.session_state.unmediated_debate.metrics_handler is None)
        and any(st.session_state.metrics[metric] for metric in st.session_state.metrics)
        or (st.session_state.unmediated_debate.metrics_handler is not None)
        and all(
            not (st.session_state.metrics[metric])
            ...
            and (
                len(
                    [
                        metric
                        for metric in st.session_state.metrics
                        if st.session_state.metrics[metric]
                    ]
                )
                != len(
                    st.session_state.unmediated_debate.metrics_handler.argument_qualities
                )
            )
        )
        or any(
            metric
            != st.session_state.unmediated_debate.metrics_handler.argument_qualities[i]
            for i, metric in enumerate(
                [m for m in st.session_state.metrics if st.session_state.metrics[m]]
            )
        )
    ):
    ...
    if st.session_state.unmediated or st.session_state.mediated:
        ...
        debates = [st.session_state.unmediated_debate, st.session_state.mediated_debate]
        ...
        for i in range(2):
            ...
            with debate_cols[2 * i]:
                ...
                st.markdown(
                    f"<h2 style='text-align: center; color: grey;'>{headers[i]}</h2>",
                    unsafe_allow_html=True,
                )
                if any(value for value in st.session_state.metrics.values()):
                    streamlit_plot_metrics(debate, metric_to_plot=metric_to_display)
                for intervention in debate.interventions:
                    if intervention.text:
                        if intervention.debater:
                            st.markdown(
                                f"👤 ({counters[i]}) **{intervention.debater.name}**: {intervention.text}"
                            )
                            counters[i] += 1
                        else:  # Mediator
...
def reset_chat():
    random.seed(SEED)
    st.session_state.unmediated_debate.preload_chat(
        debaters=st.session_state.debaters, interventions=[]
    )
    st.session_state.mediated_debate.preload_chat(
        debaters=st.session_state.debaters, interventions=[]
    )
...
def initialize_debate():
    summary_config = SummaryConfig(
        latest_messages_limit=5, debaters=st.session_state.debaters, ignore=True
    )
